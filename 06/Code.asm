...
D=M //Store input in data memory D.
@CLEAR //If input = 0 (no input), clear the screen.
D;JEQ
@FILL //If input != 0 (there is input), fill the screen.
0;JMP
...
(CLEAR)
	...
	@START //Go back to start if register count - iteration = 0. In START we reset the iteration count.
	D;JEQ
	@i //iterate
	M=M+1
