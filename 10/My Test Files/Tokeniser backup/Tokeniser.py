...
def tokeniser(file, symbol, keyword, numbers):
    ...
    for line in file:
        ...
        for character in line:
            if flag == 1: #If the character is not part of a string.

                if character.isalpha() == True or character.isnumeric() == True: #If the character is a-z or 0-9 and flag is 1 (i.e word is not part of a string), concatenate it into a word.
                    word = word + character
                elif character.isalpha() == False and character.isnumeric() == False and len(word) > 0: #Append the word to the list when it's ended.
                    wlist.append(word)
                    word = ""

                if character == '"': #Prepare to handle string. Make strings start with " so they can be identified as such.
                    flag = 0
                    word = '"'
                elif character in symbol:
                    wlist.append(character)

            else: #If the character is part of a string.
                if character == '"': #Once the end of the string is reached, set flag back to 1.
                    flag = 1
                    wlist.append(word)
                    word = ""
                else:
