...
def subroutineBody(indent, itemcounter, fileLineList, ofile, classSymbolTable, subrSymbolTable, voidFlag, constructorFlag, methodFlag, className):
    ...
    tokentype = getTokenType(fileLineList[itemcounter])
    token = getToken(fileLineList[itemcounter])
    itemcounter = statements(indent, itemcounter, fileLineList, ofile, classSymbolTable, subrSymbolTable, voidFlag, className)
...
def statements(indent, itemcounter, fileLineList, ofile, classSymbolTable, subrSymbolTable, voidFlag, className):

    tokentype = getTokenType(fileLineList[itemcounter])
    token = getToken(fileLineList[itemcounter])
    while tokentype == "keyword" and token in ("let", "if", "while", "do", "return"):
        if token == "let":
            itemcounter = letStatement(indent, itemcounter, fileLineList, ofile, classSymbolTable, subrSymbolTable, className)
        elif token == "if":
            itemcounter = ifStatement(indent, itemcounter, fileLineList, ofile, classSymbolTable, subrSymbolTable, voidFlag, className)
        elif token == "while":
            itemcounter = whileStatement(indent, itemcounter, fileLineList, ofile, classSymbolTable, subrSymbolTable, voidFlag, className)
        elif token == "do":
            itemcounter = doStatement(indent, itemcounter, fileLineList, ofile, classSymbolTable, subrSymbolTable, className)
        elif token == "return":
            itemcounter = returnStatement(indent, itemcounter, fileLineList, ofile, classSymbolTable, subrSymbolTable, voidFlag, className)
...
def letStatement(indent, itemcounter, fileLineList, ofile, classSymbolTable, subrSymbolTable, className):
    ...
    if tokentype == "identifier":
        ...
        if mSegment == "field": #If the segment is "field", we want to pop to "this" (pop to the object's fields).
            mSegment = "this"
        mIndex = str(GetIndex(mSymbolTable, token))
    ...
    if tokentype == "symbol" and token == "[":
        ...
        if tokentype in ("integerConstant", "stringConstant", "identifier") or (tokentype == "keyword" and token in ("true", "false", "null", "this")) or (tokentype == "symbol" and token in ("(", "-", "~")):
            itemcounter = expression(indent, itemcounter, fileLineList, ofile, classSymbolTable, subrSymbolTable, className)
        else:
    ...
    #Pop expression result to correct memory segment.
    ofile.write("pop " + mSegment + " " + mIndex + "\n")

    return itemcounter

def ifStatement(indent, itemcounter, fileLineList, ofile, classSymbolTable, subrSymbolTable, voidFlag, className):
...
def whileStatement(indent, itemcounter, fileLineList, ofile, classSymbolTable, subrSymbolTable, voidFlag, className):
...
def doStatement(indent, itemcounter, fileLineList, ofile, classSymbolTable, subrSymbolTable, className):
    ...
    if tokentype == "identifier" and nexttokentype == "symbol" and nexttoken == "." and varName == False:
        tokentype = "className"
        itemcounter = subroutineCall(indent, itemcounter, fileLineList, ofile, classSymbolTable, subrSymbolTable, tokentype, className)
...
def returnStatement(indent, itemcounter, fileLineList, ofile, classSymbolTable, subrSymbolTable, voidFlag, className):
...
def expression(indent, itemcounter, fileLineList, ofile, classSymbolTable, subrSymbolTable, className):

    #Add the term statement. Obligatory.
    tokentype = getTokenType(fileLineList[itemcounter])
    token = getToken(fileLineList[itemcounter])
    if tokentype in ("integerConstant", "stringConstant", "identifier") or (tokentype == "keyword" and token in ("true", "false", "null", "this")) or (tokentype == "symbol" and token in ("(", "-", "~")):
        itemcounter = term(indent, itemcounter, fileLineList, ofile, classSymbolTable, subrSymbolTable, className)
    else:
...
def term(indent, itemcounter, fileLineList, ofile, classSymbolTable, subrSymbolTable, className):
...
#NOTE: Add string functionality
    ...
    #term is varName, and is neither followed by an "[" nor an "(".             (in the former case it would be an array, and in the latter case it would be a subroutineCall).
        ...
        mIndex = str(GetIndex(mSymbolTable, token))
        ofile.write("push " + mSegment + " " + mIndex + "\n")
...
def subroutineCall(indent, itemcounter, fileLineList, ofile, classSymbolTable, subrSymbolTable, tokentype, className):
    ...
    #subroutineCalls done on a subroutine are calling methods.
        ...
        ofile.write("push pointer 0\n")
        methodCallFlag = True
    #Note that subroutineCalls done on a variable are calling methods!
    ...
    itemcounter = expressionList(indent, itemcounter, fileLineList, ofile, classSymbolTable, subrSymbolTable, callstring, methodCallFlag, className)
...
def expressionList(indent, itemcounter, fileLineList, ofile, classSymbolTable, subrSymbolTable, callstring, methodCallFlag, className):
