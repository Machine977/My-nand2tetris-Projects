...
def subroutineDec(indent, itemcounter, fileLineList, ofile, classSymbolTable, subrSymbolTable, className):
    ...
    token = getToken(fileLineList[itemcounter])
    if tokentype == "symbol" and token == "{":
        itemcounter = subroutineBody(indent, itemcounter, fileLineList, ofile, classSymbolTable, subrSymbolTable, voidFlag, constructorFlag, subroutineType, className)
    else:
...
def subroutineBody(indent, itemcounter, fileLineList, ofile, classSymbolTable, subrSymbolTable, voidFlag, constructorFlag, subroutineType, className):
    ...
    if constructorFlag == True:
        ...
        #If the subroutine is a method, argument 0 will be THIS, therefore when a function runs a method, pointer 0 (base address for selected object) must be set to that of the object fed into the method.
    ...
    tokentype = getTokenType(fileLineList[itemcounter])
    token = getToken(fileLineList[itemcounter])
    itemcounter = statements(indent, itemcounter, fileLineList, ofile, classSymbolTable, subrSymbolTable, voidFlag, subroutineType, className)
...
def statements(indent, itemcounter, fileLineList, ofile, classSymbolTable, subrSymbolTable, voidFlag, subroutineType, className):
    tokentype = getTokenType(fileLineList[itemcounter])
    token = getToken(fileLineList[itemcounter])

    while tokentype == "keyword" and token in ("let", "if", "while", "do", "return"):
        if token == "let":
            itemcounter = letStatement(indent, itemcounter, fileLineList, ofile, classSymbolTable, subrSymbolTable, subroutineType, className)
        elif token == "if":
            itemcounter = ifStatement(indent, itemcounter, fileLineList, ofile, classSymbolTable, subrSymbolTable, subroutineType, voidFlag, className)
        elif token == "while":
            itemcounter = whileStatement(indent, itemcounter, fileLineList, ofile, classSymbolTable, subrSymbolTable, subroutineType, voidFlag, className)
        elif token == "do":
            itemcounter = doStatement(indent, itemcounter, fileLineList, ofile, classSymbolTable, subrSymbolTable, subroutineType, className)
        elif token == "return":
            itemcounter = returnStatement(indent, itemcounter, fileLineList, ofile, classSymbolTable, subrSymbolTable, voidFlag, subroutineType, className)
        tokentype = getTokenType(fileLineList[itemcounter])
        token = getToken(fileLineList[itemcounter])

    return itemcounter

def letStatement(indent, itemcounter, fileLineList, ofile, classSymbolTable, subrSymbolTable, subroutineType, className):
    ...
    if tokentype == "symbol" and token == "[":
        ...
        if tokentype in ("integerConstant", "stringConstant", "identifier") or (tokentype == "keyword" and token in ("true", "false", "null", "this")) or (tokentype == "symbol" and token in ("(", "-", "~")):
            ...
            itemcounter = expression(indent, itemcounter, fileLineList, ofile, classSymbolTable, subrSymbolTable, className, subroutineType)
        else:
...
def ifStatement(indent, itemcounter, fileLineList, ofile, classSymbolTable, subrSymbolTable, subroutineType, voidFlag, className):
...
def whileStatement(indent, itemcounter, fileLineList, ofile, classSymbolTable, subrSymbolTable, subroutineType, voidFlag, className):
...
def doStatement(indent, itemcounter, fileLineList, ofile, classSymbolTable, subrSymbolTable, subroutineType, className):
    ...
    if tokentype == "identifier" and nexttokentype == "symbol" and nexttoken == "." and varName == False:
        tokentype = "className"
        itemcounter = subroutineCall(indent, itemcounter, fileLineList, ofile, classSymbolTable, subrSymbolTable, tokentype, className, subroutineType)
...
def returnStatement(indent, itemcounter, fileLineList, ofile, classSymbolTable, subrSymbolTable, voidFlag, subroutineType, className):
...
def expression(indent, itemcounter, fileLineList, ofile, classSymbolTable, subrSymbolTable, className, subroutineType):

    #Add the term statement. Obligatory.
    tokentype = getTokenType(fileLineList[itemcounter])
    token = getToken(fileLineList[itemcounter])
    if tokentype in ("integerConstant", "stringConstant", "identifier") or (tokentype == "keyword" and token in ("true", "false", "null", "this")) or (tokentype == "symbol" and token in ("(", "-", "~")):
        itemcounter = term(indent, itemcounter, fileLineList, ofile, classSymbolTable, subrSymbolTable, className, subroutineType)
    else:
...
def term(indent, itemcounter, fileLineList, ofile, classSymbolTable, subrSymbolTable, className, subroutineType):
    ...
    #term is varName, and is neither followed by an "[" nor an "(".             (in the former case it would be an array, and in the latter case it would be a subroutineCall).
        ...
        mSegment = GetKind(mSymbolTable, token)
        if mSegment == "field":
            #The field should be passed as the actual object field if the function is a method.
            if subroutineType in ("method", "constructor"):
        ...
        if mSegment == "argument" and subroutineType == "method":
            mIndex += 1
            #print("term " + str(subroutineType))
        ofile.write("push " + mSegment + " " + str(mIndex) + "\n")
...
def subroutineCall(indent, itemcounter, fileLineList, ofile, classSymbolTable, subrSymbolTable, tokentype, className, subroutineType):
    ...
    itemcounter = expressionList(indent, itemcounter, fileLineList, ofile, classSymbolTable, subrSymbolTable, callstring, methodCallFlag, subroutineType, className)
...
def expressionList(indent, itemcounter, fileLineList, ofile, classSymbolTable, subrSymbolTable, callstring, methodCallFlag, subroutineType, className):
